...
class BenchmarkResult:
    ...
    def _idealDCG(self) -> float:
        ...
        dcg = 0
        i = 1
        results = self.validResults.copy()
    ...
    def getPrecisionAtRecallLevel(self, recall_level: float, interpolated: bool = False) -> float:
        ...
        """
        if not self.results or recall_level == 0:
            return 0
...
class Benchmark:
    ...
    def run(self):
        ...
        for benchmarkQuery in self.queries:
            start = time.time()
            results: list[str] = self.index.query(benchmarkQuery.query, limit=len(benchmarkQuery.expectedResults))
            end = time.time()
